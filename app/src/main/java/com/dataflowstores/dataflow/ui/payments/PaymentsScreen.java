...
import com.dataflowstores.dataflow.databinding.PaymentsBinding;
...
public class PaymentsScreen extends BaseActivity implements MyDialogCloseListener, LocationListener {
    ...
    private void setupViews() {
        ...
        viewModel.customerBalanceLiveData.observe(this, customerBalance -> {
            ...
            binding.showProgress.setVisibility(View.GONE);
            startActivity(new Intent(PaymentsScreen.this, PrintPayment.class));
            finish();
        });
    }
    ...
    private boolean validateData() {
        if (binding.getSupplier.getText().toString().isEmpty() || App.customer.getDealerName() == null) {
            binding.getSupplier.setError("إسم المورد مطلوب");
            return false;
    ...
    private void searchButtons() {
        ...
        binding.searchSupplier.setOnClickListener(view -> {
            if (App.isNetworkAvailable(this))
                invoiceVM.getSupplier(uuid, binding.getSupplier.getText().toString(), App.currentUser.getWorkerBranchISN(), App.currentUser.getWorkerISN());
            else {
                App.noConnectionDialog(this);
            }
            BottomSheetFragment bottomSheetFragment = new BottomSheetFragment();
...
//                     ...
            ...
            case REQUEST_CHECK_SETTINGS:
                switch (resultCode) {
                    case Activity.RESULT_OK: {
                        getLocation(PaymentsScreen.this);
    ...
    public void createReceipt() {
        ...
        viewModel.paymentModelMutableLiveData.observe(this, receiptModel -> {
            App.receiptModel = receiptModel;
            if (App.customer.getDealerName() != null && App.currentUser.getMobileShowDealerCurrentBalanceInPrint() == 1) {
                ReceiptData receiptData = receiptModel.getData().get(0);
                viewModel.getCustomerBalance(uuid, String.valueOf(App.customer.getDealer_ISN()), String.valueOf(App.customer.getBranchISN()), String.valueOf(App.customer.getDealerType()),
