...
import com.dataflowstores.dataflow.ui.listeners.MyButtonClickListener;
...
public abstract class SwipeHelper extends ItemTouchHelper.SimpleCallback
{
    ...
    private View.OnTouchListener onTouchListener = new View.OnTouchListener()
    {
        ...
        public boolean onTouch(View view, MotionEvent motionEvent) {
            ...
            Point point = new Point((int) motionEvent.getRawX(), (int)
                    motionEvent.getRawY());
            ...
            if(swipeViewHolder != null)
            {
                        ...
                        motionEvent.getAction() == MotionEvent.ACTION_MOVE)
                {
                    ...
                    {
                        removeQueue.add(swipePosition); swipePosition = -1;
                        recoverSwipedItem();
                    }
                }
                return false;
    ...
    {
        super(0, ItemTouchHelper.LEFT); this.recyclerView =
            recyclerView; this.buttonList = new ArrayList<>();

        this.gestureDetector = new GestureDetector(context,
                gestureListener);
        this.recyclerView.setOnTouchListener(onTouchListener);
        this.buttonBuffer = new HashMap<>();
        this.buttonWidth = buttonWidth; removeQueue = new LinkedList() {


        private boolean add(Integer integer)
        {
            if(contains(integer))
                return false;
            else
    ...
    private Bitmap drawableToBitmap(Drawable d)
    {
        ...
        {
            return Bitmap.createScaledBitmap(((BitmapDrawable)
                    d).getBitmap(), 160, 160, true);
        }

        Bitmap bitmap = Bitmap.createBitmap(d.getIntrinsicWidth(),
                d.getIntrinsicHeight(), Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(bitmap); d.setBounds(0, 0,
            canvas.getWidth(),
            canvas.getHeight());
        d.draw(canvas);
    ...
    @Override public void onSwiped(@NonNull RecyclerView.ViewHolder viewHolder, int direction)
    {
        ...
        if(swipePosition != pos)
            removeQueue.add(swipePosition); swipePosition = pos;
    ...
    @Override public float getSwipeEscapeVelocity(float defaultValue)
    {
        return 0.1f * defaultValue;
    }

    @Override public float getSwipeVelocityThreshold(float defaultValue) {
            ...
            @NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive)
    {
        ...
        if(actionState == ItemTouchHelper.ACTION_STATE_SWIPE)
        {
            if(dX < 0)
            {
                ...
                translationX = dX * buffer.size() * buttonWidth /
                        itemView.getWidth();
    ...
    private void drawButton(Canvas c, View itemView, List buffer, int pos, float translationX)
    {
        ...
        for(int i=0; i<buffer.size(); i++)
        {
            ...
            MyButton myButton = (MyButton) buffer.get(i);
            myButton.onDraw(c, new RectF(left, itemView.getTop(), right, itemView.getBottom()), pos); right = left;
        }
    }
